...
class TheaterViewsTest(TestCase):
    ...
    def test_create_theatre_hall(self):
        ...
        response = self.client.post(reverse('theatrehall-list'), data, format='json')
    ...
    def test_play_detail(self):
        ...
        response = self.client.get(reverse('play-detail', kwargs={'pk': self.play.id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['title'], 'Hamlet')
        self.assertEqual(response.data['description'], "Shakespeare's masterpiece")
